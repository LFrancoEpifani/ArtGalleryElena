...
export default function ArtDetails() {
  ...
  useEffect(() => {
    ...
    const paintRandom = randomPaint(
      artworks.filter((item) => item.id.toString() !== artId)
    );
      ...
      <div className="lg:flex p-10 justify-evenly items-center">
        <div className="flex flex-col lg:flex-row gap-4 lg:gap-8 items-center">
          <div className="bg-black">
            <img
              className="object-contain w-90 shadow-xl border border-black 2xl:w-100 2xl:h-100vh"
              src={selectedImage}
              alt=""
            />
          </div>
          {art.image && art.house ? (
            <div className="flex lg:flex-col items-start gap-3">
              <img
                className="h-12 w-12 object-cover cursor-pointer 2xl:h-20 2xl:w-20"
                src={art.image}
                alt=""
              ...
              <img
                ...
                src={art.house}
                alt=""
                onClick={() => handleImageClick(art.house)}
              />
            </div>
          ) : (
            <img
              className="h-12 w-12 object-cover cursor-pointer 2xl:h-20 2xl:w-20"
              src={art.image || art.house}
              alt=""
              onClick={() => handleImageClick(art.image || art.house)}
            />
          )}
        </div>
        <div className="my-4">
          ...
          <div className="my-2">
            ...
            <p className="text-[10px] text-gray-500 2xl:text-[15px] 2xl:my-4">
              {art.description}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4 my-14">
            <button
              onClick={() => {handleAddToCart(); setIsOpen(true);}}
          ...
          {randomArts.map((randomArt) => (
            ...
            >
              <div key={randomArt.id}>
                <img
                  className="object-contain w-35 lg:w-50 shadow-xl shadow-gray-300 rounded-sm"
                  src={randomArt.image}
                  alt={randomArt.name}
                />
              </div>
